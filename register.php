...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi input tidak boleh kosong (sekarang email_ortu juga wajib)
    if (!empty($nama_lengkap) && !empty($kelas) && !empty($uid_rfid) && !empty($email_ortu)) {
        // Validasi format email
        if (!filter_var($email_ortu, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($stmt) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    if (mysqli_errno($koneksi) == 1062) {
                        $pesan = "Error: UID RFID sudah terdaftar.";
                        $pesan_type = "danger";
                    } else {
                        $pesan = "Error: " . mysqli_stmt_error($stmt);
                        $pesan_type = "danger";
                    }
                }
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .form-label .required {
            color: #ef4444; /* Red-500 for required asterisk */
        }
    </style>
</head>
        ...
        <div class="max-w-7xl mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                ...
                <div class="hidden md:flex space-x-8">
                    ...
                    <a href="register.php" class="text-blue-600 font-bold transition duration-300 flex items-center space-x-2 border-b-2 border-blue-600">
            ...
            <?php if ($pesan): ?>
                <div class="mb-8 p-6 rounded-2xl shadow-lg <?php echo 'alert-' . $pesan_type; ?> animate-slideInUp border-l-4">
                    <div class="flex items-center">
                        <i class="fas fa-<?php echo $pesan_type === 'success' ? 'check-circle' : ($pesan_type === 'warning' ? 'exclamation-triangle' : 'times-circle'); ?> text-2xl mr-3"></i>
                ...
                <form action="register.php" method="POST" class="space-y-6">
                    <div>
                        <label for="nama_lengkap" class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-user-alt text-blue-600 mr-2"></i>Nama Lengkap <span class="required">*</span>
                        </label>
                        <input 
                            type="text" 
                            id="nama_lengkap" 
                    ...
                    <div>
                        <label for="uid_rfid" class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-id-card text-purple-600 mr-2"></i>UID Kartu RFID <span class="required">*</span>
                        </label>
                        <input 
                            type="text" 
                            id="uid_rfid" 
                            ...
                            autofocus
                        />
                        <p class="text-sm text-gray-500 mt-2 ml-1">Tempelkan kartu RFID pada reader untuk mengisi otomatis</p>
                    </div>

                    <div>
                        <label for="email_ortu" class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-envelope text-orange-600 mr-2"></i>Email Orang Tua <span class="required">*</span>
                        </label>
                        <input 
                            type="email" 
                            id="email_ortu" 
                            ...
                            required
                        />
                        <p class="text-sm text-gray-500 mt-2 ml-1">Email akan digunakan untuk notifikasi absensi siswa</p>
                    </div>
